...
@ui.decorators.page<MobileStockChangeByIdentifierDestination>({
    title: 'Stock change',
    ...
    async onLoad() {
        const _siteCode = this.$.storage.get('mobile-selected-stock-site') as string;
...
export class MobileStockChangeByIdentifierDestination extends ui.Page {
    @ui.decorators.pageAction<MobileStockChangeByIdentifierDestination>({
        title: 'Next',
        buttonType: 'primary',
        shortcut: ['f3'],
        isDisabled: true,
    ...
    @ui.decorators.section<MobileStockChangeByIdentifierDestination>({
        isTitleHidden: true,
    })
    section: ui.containers.Section;

    @ui.decorators.block<MobileStockChangeByIdentifierDestination>({
        parent() {
            return this.section;
        },
        isTitleHidden: true,
     ...
     * Technical fields
     */
    @ui.decorators.textField<MobileStockChangeByIdentifierDestination>({
        isTransient: true,
        isReadOnly: true,
    })
    ...
    @ui.decorators.referenceField<MobileStockChangeByIdentifierDestination, Location>({
        parent() {
            return this.block;
        },
            ...
            if (this.locationDestination.value?.code) {
                if (_stockChangeLines) {
                    ...
                    if (!_sameLocationOrStatus) {
                        ...
                        this.$.showToast(
                            ui.localize(
                                '@sage/x3-stock/pages__mobile_stock_change_by_identifier_destination__notification__invalid_stock_location_destination__error',
    ...
    @ui.decorators.selectField<MobileStockChangeByIdentifierDestination>({
        parent() {
            return this.block;
        },
            ...
            if (this.statusDestination.value) {
                if (_stockChangeLines) {
                    ...
                    if (!_sameLocationOrStatus) {
                        ...
                        this.$.showToast(
                            ui.localize(
                                '@sage/x3-stock/pages__mobile_stock_change_by_identifier_destination__notification__invalid_stock_status_destination__error',
    ...
    private async _getStockStatuses(): Promise<string[]> {
        try {
            ...
            this.$.dialog.message(
                'error',
                ui.localize(
                    '@sage/x3-stock/pages__mobile_stock_change_by_identifier_destination__notification__invalid_stock_status_error',
                    'No stock status',
                ),
                String(err),
            );
